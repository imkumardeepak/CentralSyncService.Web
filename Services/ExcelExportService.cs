...
namespace Web.Services
{
    public class ExcelExportService
    {
        ...
        public byte[] ExportProductWiseTransfer(List<Core.DTOs.ProductWiseDailyTransferRecord> data, DateTime selectedDate)
        {
            using var workbook = new XLWorkbook();
            var worksheet = workbook.Worksheets.Add("Product Transfer");

            ApplyHeaderStyle(worksheet, "Product Wise Daily Transfer", selectedDate);

            var header1 = new[] { "", "SAP Code / Product", "", "", "FROM (Issue)", "", "", "TO (Receipt)", "" };
            var row1 = worksheet.Row(4);
            for (int i = 0; i < header1.Length; i++)
            {
                var cell = row1.Cell(i + 1);
            ...
            var subHeaders = new[] { "SAP Code", "Product Name", "Batch", "Total", "Read", "No Read", "Total", "Read", "No Read" };
            var subHeaderRow = worksheet.Row(5);
            for (int i = 0; i < subHeaders.Length; i++)
            {
                var cell = subHeaderRow.Cell(i + 1);
            ...
            foreach (var item in data)
            {
                ...
                worksheet.Cell(row, 3).Value = item.Batch;
                worksheet.Cell(row, 4).Value = item.TotalIssue;
                worksheet.Cell(row, 5).Value = item.IssueRead;
                worksheet.Cell(row, 6).Value = item.IssueNoRead;
                worksheet.Cell(row, 7).Value = item.TotalReceipt;
                worksheet.Cell(row, 8).Value = item.ReceiptRead;
                worksheet.Cell(row, 9).Value = item.ReceiptNoRead;
        ...
        public byte[] ExportDailySummary(List<Core.DTOs.DailySummaryRecord> data, DateTime startDate, DateTime endDate)
        {
            using var workbook = new XLWorkbook();
            var worksheet = workbook.Worksheets.Add("Daily Summary");

            var titleDate = startDate == endDate ? startDate.ToString("dd MMM yyyy") : $"{startDate:dd MMM} - {endDate:dd MMM yyyy}";
            ApplyHeaderStyle(worksheet, "Daily Summary Report", titleDate);

            var headers = new[] { "Date", "Issue Total", "Issue Read", "Issue No Read", "Receipt Total", "Receipt Read", "Receipt No Read" };
            var headerRow = worksheet.Row(4);
            for (int i = 0; i < headers.Length; i++)
            {
                var cell = headerRow.Cell(i + 1);
            ...
            foreach (var item in data)
            {
                worksheet.Cell(row, 1).Value = item.ReportDate.ToString("dd-MM-yyyy");
                worksheet.Cell(row, 2).Value = item.TotalIssue;
                worksheet.Cell(row, 3).Value = item.IssueRead;
                worksheet.Cell(row, 4).Value = item.IssueNoRead;
                worksheet.Cell(row, 5).Value = item.TotalReceipt;
                worksheet.Cell(row, 6).Value = item.ReceiptRead;
                worksheet.Cell(row, 7).Value = item.ReceiptNoRead;
        ...
        public byte[] ExportNoReadAnalysis(List<Core.DTOs.NoReadAnalysisRecord> data, DateTime selectedDate)
        {
            using var workbook = new XLWorkbook();
            var worksheet = workbook.Worksheets.Add("NO READ Analysis");

            ApplyHeaderStyle(worksheet, "NO READ Analysis Report", selectedDate);

            var headers = new[] { "Scanner", "Plant", "Line Code", "Hour", "No Read Count", "No Read %" };
            var headerRow = worksheet.Row(4);
            for (int i = 0; i < headers.Length; i++)
            {
                var cell = headerRow.Cell(i + 1);
            ...
            foreach (var item in data)
            {
                worksheet.Cell(row, 1).Value = item.Scanner;
                worksheet.Cell(row, 2).Value = item.Plant ?? "";
                worksheet.Cell(row, 3).Value = item.LineCode ?? "";
                worksheet.Cell(row, 4).Value = item.Hour;
                worksheet.Cell(row, 5).Value = item.NoReadCount;
                worksheet.Cell(row, 6).Value = item.NoReadPercent;

                ApplyDataRowStyle(worksheet, row, 6);

                if (item.NoReadPercent > 10)
                    worksheet.Cell(row, 6).Style.Font.FontColor = XLColor.FromHtml("#DC2626");
                else if (item.NoReadPercent > 5)
                    worksheet.Cell(row, 6).Style.Font.FontColor = XLColor.FromHtml("#F59E0B");
        ...
        private void ApplyHeaderStyle(IXLWorksheet worksheet, string title, string dateRange)
        {
            worksheet.PageSetup.PageOrientation = XLPageOrientation.Landscape;
            worksheet.PageSetup.FitToPages(1, 1);
            worksheet.PageSetup.Margins.Left = 0.5;
            ...
            var dateCell = worksheet.Cell(3, 1);
            dateCell.Value = $"Report Period: {dateRange}";
            dateCell.Style.Font.FontSize = 10;
            dateCell.Style.Font.FontColor = XLColor.FromHtml("#6B7280");
            dateCell.Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
