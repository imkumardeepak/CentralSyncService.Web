...
namespace Web.Services
{
    ...
    /// Pulls data from FROM and TO plant local databases and inserts directly into SorterScans_Sync
    /// </summary>
        ...
        private async Task PerformSyncAsync(CancellationToken cancellationToken)
        {
            using (var scope = _serviceScopeFactory.CreateScope())
            {
                ...
                foreach (var plant in _plantConfigs.Where(p => p.PlantType == "FROM"))
                {
                    try
                    {
                        _logger.LogInformation("Connecting to FROM plant: {PlantName} at {IpAddress}",
                            plant.PlantName, plant.IpAddress);
                    ...
                    catch (Exception ex)
                    {
                        ...
                        _logger.LogError(ex, "Error fetching FROM from {PlantName}", plant.PlantName);
                ...
                // STEP 3: Insert FROM records directly into SorterScans_Sync
                foreach (var record in fromRecords)
                {
                    try
                    {
                        await syncRepo.InsertSorterScanAsync(record).ConfigureAwait(false);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError(ex, "Error inserting FROM record {Id}: {Barcode}", record.Id, record.Barcode);
                    }
                }

                // STEP 4: Insert TO records directly into SorterScans_Sync
                foreach (var record in toRecords)
                {
                    ...
                    {
                        _logger.LogError(ex, "Error inserting TO record {Id}: {Barcode}", record.Id, record.Barcode);
                    }
                }
                ...
                _logger.LogInformation("Sync complete: {FromCount} FROM, {ToCount} TO synced", fromRecords.Count, toRecords.Count);
            }
        }
